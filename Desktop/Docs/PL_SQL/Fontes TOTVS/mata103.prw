	...
	If lContinua
		...
		If nOpc == 1 .And. (l103Inclui.Or.l103Class.Or.l103Exclui)	.And. If(Type("l103GAuto") == "U" ,.T.,l103GAuto)
			...
			If A103Trava() .And. IIf(lIntegGFE .And. l103Exclui,ExclDocGFE(),.T.)
				...
				If l103Auto .And. Type("lTemF1GFE")<>"U" .And. lIntegGFE
...
Local nPosItPC     := aScan(aHeader ,{|x| AllTrim(x[2])=="D1_ITEMPC"})
Local nPosOP       := aScan(aHeader ,{|x| AllTrim(x[2])=="D1_OP"})
Local nPosIDB6     := aScan(aHeader ,{|x| AllTrim(x[2])=="D1_IDENTB6"})
Local nPosTes      := 0
Local nPosAC	   := 0
Local nQTDDev      := 0
Local nXCDanfe     := 0
	...
	If !lDeleta
		...
		For nX := 1 to Len(aCols)
			...
			If !aCols[nx][Len(aHeader)+1]
				...
				SC2->(MsSeek(xFilial("SC2")+aCols[nX][nPosOP]))
					Aadd(aOPBenef,{aCols[nX][nPosOP],SC2->C2_QUANT})					
				Endif
...
Local lSimpNac	:= MaFisRet(,"NF_SIMPNAC") == "1"
...
Static Function A103Adiant(lUsaAdi)
	...
	If Len(aPedAdt) > 0
		For nCnt := 1 to Len(aPedAdt)
			...
			For nCnt1 := 1 To Len(aPedidos)
				...
				If SE2->(Recno()) = aPedidos[nCnt1][2]
					If SE2->E2_SALDO >= SaldoTit(SE2->E2_PREFIXO,SE2->E2_NUM,SE2->E2_PARCELA,SE2->E2_TIPO,SE2->E2_NATUREZ,"P",SE2->E2_FORNECE,nMoedaCor,dDataBase,RetDtBxPA(),SE2->E2_LOJA,,0,1)
		...
		If nValAdt == 0 .And. lUsaAdi
			...
			lRet := .F.
		ElseIf cPaisLoc == "BRA" .And. nValMov < nValAdt .And. lUsaAdi .And. lVldAdt //Se movimentou menos banco do que adiantou, existe adto sem mov (SOMENTE BRASIL)
		...
		Else
			...
			Else
				If !lVldAdt .And. !lTemMov //Se o par‚metro MV_VLDADT estiver desativado e n„o possui movimentaÁ„o bancaria
