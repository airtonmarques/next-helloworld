select distinct 
      ...
      bairro.nom_bairro bairro,
			m.nom_municipio cidade,
			m.sgl_uf uf,
			bend.num_cep        
     ...
     left join ts.tipo_dependencia td on td.cod_dependencia = b.cod_dependencia
		 left join ts.beneficiario_endereco bend on bend.cod_entidade_ts = b.cod_entidade_ts_tit
		                                         and bend.num_seq_end = (select max(bend1.num_seq_end)
                                                                     ...
                                                                     where bend1.cod_entidade_ts = bend.cod_entidade_ts)
		 left join ts.bairro bairro on bairro.cod_bairro = bend.cod_bairro
		 left join ts.municipio m on m.cod_municipio = bend.cod_municipio

where 1 = 1
and ce.num_contrato IN (15277, 33283, 35697) --INFRAERO 
--MEC        
