--B = ação de beneficiário  C = ação de contrato
select distinct
       ...
       decode(aj.ind_faturamento, 'S', 'Sim', 'Não') faturar,
       decode(aj.ind_inadimplencia, 'S', 'Sim', 'Não') aplicarInadimplencia,
       aj.polo_ativo poloAtivo,
       aj.polo_passivo_1 poloPassivo1,
       aj.polo_passivo_2 poloPassivo2,
       
       (select decode(itensResp.Ind_Resposta, 'S', 'Sim', 'N', 'Não', itensResp.Txt_Resposta)
