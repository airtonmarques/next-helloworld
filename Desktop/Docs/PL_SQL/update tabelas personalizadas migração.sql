...
from ts.entidade_sistema es
     ...
     join ts.faixa_etaria fe on fe.tipo_faixa = 466 --and fe.idade_inicial = 0
		 join ts.valor_fe_contrato fec on fec.cod_ts_contrato = ce.cod_ts_contrato
                                   and fec.cod_plano = t.codigo_do_plano
where c.operadora like '%GNDI%'
      --and t.tipo_tabela_faixa_etaria = 'UNICA'
      and t.tipo_tabela_faixa_etaria IN ('UNICA')
