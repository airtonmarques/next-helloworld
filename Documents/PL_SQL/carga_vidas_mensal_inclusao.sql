...
suc.nome_sucursal,
     ...
     (select sys_get_vlr_cont_fe_fnc(2, ce.cod_ts_contrato, p.cod_plano, b.tipo_associado, SYS_GET_IDADE_FNC(be.data_nascimento, b.data_inclusao), b.data_inclusao) from dual)
 as VALOR_COMERCIAL,

     (select sys_get_vlr_cont_fe_fnc(1, ce.cod_ts_contrato, p.cod_plano, b.tipo_associado, SYS_GET_IDADE_FNC(be.data_nascimento, b.data_inclusao), b.data_inclusao) from dual)
 as VALOR_NET,
...
-- Inclusão
and b.data_inclusao between to_date('01/03/2023', 'dd/mm/yyyy') and last_day('01/03/2023')
...
set bsp.data_exclusao_anterior = (select max(b.data_exclusao)
                                ...
                                where be.num_cpf = to_number(bsp.cpf)
                                      ...
                                      and nvl(b.data_exclusao, to_date('31/12/3000', 'DD/MM/YYYY')) > b.data_inclusao)
where bsp.lote_inclusao =  '050520230900';
...
update all_bene_inc bsp
set bsp.data_registo_mo = (select min(o.dt_fato) 
                           from ts.ocorrencia_associado o
                           where o.cod_ocorrencia = 184
                                 and o.cod_ts = bsp.cod_ts)
where bsp.lote_inclusao =  '050520230900';
commit;
