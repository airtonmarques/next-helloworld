...
from ts.MC_ANALISE_TECNICA_HIST a
     join ts.mc_associado_mov aa on aa.num_seq_associado_mov = a.num_seq_associado_mov
		 ...
		 join ts.ppf_proposta p on p.num_seq_proposta_ts = aa.num_seq_proposta_ts
		 ...
		 join ts.ppf_situacao_proposta sp on sp.cod_situacao_proposta = p.cod_situacao_proposta
		 left join ts.regra_empresa porte on porte.tipo_empresa = p.tipo_empresa
where aa.tipo_associado = 'T'
			--and sit.cod_situacao_analise not in (11, 12)
      and p.dt_inicio_vigencia >= to_date('01/01/2022', 'dd/mm/yyyy')
			--and p.num_proposta_adesao = '23980014606'
...
where aa.tipo_associado = 'T'			
			and p.dt_inicio_vigencia >= to_date('01/01/2022', 'dd/mm/yyyy')
			--and p.num_proposta_adesao = '23980014606'--'23280004544'
)	a	 				
